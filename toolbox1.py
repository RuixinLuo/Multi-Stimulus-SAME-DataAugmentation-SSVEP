...
# 2022.12.18  trca_test() msCCA_test() msCCA_trca_Test() move test(:,:,m) to for m in range()
# 2022.12.22  remove TRCA-R scTRCA from this version of toolbox
...
## msSAME (When nTemplates=0 or 1, it equals SAME)
def PT_estimation(data, mean_target):
    """ source signal estimation

    :param data: n_channel_1, n_times
    :param mean_target: n_channel_2, n_times

    :return: PT : n_channel_2, n_channel_1
    """
    nChannel, nTimes = np.shape(data)
    X_a = data
    X = mean_target
...
def get_augment_fb_noiseAfter_ms(fs  ,f_list , phi_list, Nh_start, Nh_end, ntrail_noise, mean_temp_all , iEvent , nTemplates):
    """ msSAME
    :param fs:            the sample rate
    :param f_list:        the all frequency of reference signal
    :param phi_list:      the all phase of reference signal
    ...
    if nTemplates == 0 or nTemplates == 1: # Original SAME
        template_st = iEvent
    ...
    for j in range(template_st, template_ed, 1):
        ...
        flag = 0
        for iNh in Nh_step:
            y_sin = np.sin(2*np.pi*f*iNh*n +  iNh*np.pi*phi)
            Yf[:,flag*2] = y_sin
            y_cos = np.cos(2*np.pi*f*iNh*n +  iNh*np.pi*phi)
            Yf[:,flag*2+1] = y_cos
            flag = flag + 1
        mscca_ref[index * nTimes: (index+1) * nTimes , :] = Yf
        # templates
    ...
    PT = PT_estimation(mscca_ref.T, mscca_template.T)
    ...
    # add noise
    data_aug = np.zeros((nChannels, nTimes, ntrail_noise))
    for i_aug in range(ntrail_noise):
        # Randomly generated noise
        Datanosie = np.random.multivariate_normal(mean=np.zeros((nChannels)), cov=vars, size =  nTimes)
        data_aug[:,:,i_aug] = Z + 0.05 * Datanosie.T

    return data_aug

## msSAME with semisupervised
def get_augment_ms_SemiSupervised(fs  ,f_list , phi_list, Nh_start, Nh_end, ntrail_noise, mean_temp_all , iEvent , nTemplates):
    """ msSAME return PT compared with get_augment_fb_noiseAfter_ms
    :param fs:            the sample rate
    :param f_list:        the all frequency of reference signal
    :param phi_list:      the all phase of reference signal
    :param Nh_start: Minimum number of harmonics
